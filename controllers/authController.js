...
export const signup = async (req, res) => {
  try {
    ...
    // 🌐 Verification link (use frontend URL)
  ...
  <div style="max-width:600px;width:90%;margin:0 auto;background:#ffffff;border-radius:8px;padding-bottom:40px;text-align:center;box-shadow:0 2px 8px rgba(0,0,0,0.05);">
    ...
    <div style="text-align:left;color:#374151;font-size:15px;line-height:1.8;padding:0 8%;">
      ...
      <p style="margin:0 0 10px 0;">
        ...
        From websites to landing pages — we’ve got you covered.
      </p>
      <p style="margin:0;">
        Explore 
...
export const createProduct = async (req, res) => {
  try {
    ...
    // ✅ Step 1: Main image (already uploaded by multer-storage-cloudinary)
    if (req.files?.image?.[0]) {
      imageUrl = req.files.image[0].path; // CloudinaryStorage puts the URL here
    }

    // ✅ Step 2: Sub images
    if (req.files?.subImages?.length > 0) {
      for (const file of req.files.subImages) {
        subImageUrls.push(file.path); // Each file already has its Cloudinary URL
      }
    }

    // ✅ Step 3: Parse features and techs safely
    const safeParse = (val) => {
      if (!val) return [];
...
export const deleteProduct = async (req, res) => {
  try {
    ...
    // 2️⃣ Delete main image from Cloudinary
    if (product.imageUrl) {
      try {
    ...
    // 3️⃣ Delete subImages from Cloudinary (if they exist)
    ...
    // 5️⃣ Respond
    res.json({ message: "✅ Product deleted successfully (with images)" });
  } catch (err) {
...
export const getAllUsers = async (req, res) => {
  try {
    const general = await User.find({ role: "general" }).select("-password");
    res.status(200).json({
    ...
    res.status(500).json({
      message: "❌ Failed to fetch general users",
      error: error.message,
    });
  }
};
...
export const getCart = async (req, res) => {
  try {
    ...
    const cart = await Cart.findOne({ userId })
      .populate("products.productId", "title price imageUrl description");
